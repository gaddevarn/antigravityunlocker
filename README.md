...
Тем, что вайб-кодер ломал путём декомпилирования мой анлокер (оригинал) с помощью нейросети и теперь выдаёт как за свою собственную разработку ([подделку](https://github.com/AvenCores/open-antigravity-patcher)). Что там есть помимо моего алгоритма и насколько безопасно им пользоваться - известно только ему.
